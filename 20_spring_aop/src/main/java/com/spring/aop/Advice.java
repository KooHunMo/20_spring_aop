...
/*
	...
	- 프로젝트 개발 과정에서 핵심 기능 외에 추가적이고, 다양한 부가(공통) 기능이 필요하다. (로깅,보안,트랜젝션,테스트)
																						   (로깅,보안,트랜젝션,테스트(코드) 같은 것들이 관점이다.- 없어도 돌아가는 것들?)
...
public class Advice {
	...
	private static Logger logger = LoggerFactory.getLogger(Advice.class); 	//로거는 slf4j 사용 - logger는 안외워도 됨
																			// 홈컨트롤러에 프로젝트 새로만들면 있음 
																			// 클래스 이름만 바꾸면 됨
	...
	//*: 리턴타입 꼭 한칸 띄고 //패키지명.클래스명(*).메서드 
	// :를 실행하면 어떤 메소드건 간에 그 앞에 before메서드가 먼저 실행된다
	// 메서드 앞,뒤,앞뒤에서 실행이 가능하다.
